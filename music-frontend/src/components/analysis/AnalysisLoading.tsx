...
export function AnalysisLoading({ progress, step, partial }: AnalysisLoadingProps) {
  if (partial) {
    console.log('Partial results length:', Object.keys(partial).length);
    console.log('Partial metadata:', partial.metadata);
  }
  return (
    <PageShell>
    <main className="mx-auto flex max-w-6xl flex-1 flex-col gap-6 px-4 py-8 text-app">
      ...
      {!partial || (Object.keys(partial).length === 0) ? (
        ...
        <div className="space-y-4">
          {partial.metadata && (
            (console.log('Attempting to render metadata:', partial.metadata),
          ...
          {partial.stems && Object.keys(partial.stems).length > 0 && (
            (console.log('Attempting to render stems:', partial.stems),
            <div className="p-4 rounded-lg border border-app bg-app-elevated">
          ...
          {partial.chords && partial.chords.length > 0 && (
            (console.log('Attempting to render chords:', partial.chords),
            <div className="p-4 rounded-lg border border-app bg-app-elevated">
          ...
          {partial.notes && Object.keys(partial.notes).length > 0 && (
            (console.log('Attempting to render notes:', partial.notes),
            <div className="p-4 rounded-lg border border-app bg-app-elevated">
